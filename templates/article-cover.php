...
<header>
	...
	// Get our custom tagline
	$tagline = get_post_meta(get_the_ID(),"mog_tagline",true);
	if (!empty ($tagline)) {
		echo '<h2>'.$tagline.'</h2>';
	...
	<?php else : ?>
			<?php
			$authors =  wp_get_object_terms(get_the_ID(), 'author' , array('orderby' => 'count', 'order' => 'DESC'));
			foreach ($authors as $author) {
				$author_name = $author->name;?>
			<address><?php echo $author_name;?></address>
			<?php
			}
			?>
	<?php endif; ?>
